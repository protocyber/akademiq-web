...
function PublishedReportCardShell() {
  ...
  return (
    <SidebarLayout schoolName={tenant.data.school_name} userName={me.data.full_name} userEmail={me.data.email} isLoggingOut={logout.isPending} onLogout={async () => { await logout.mutateAsync(); router.push("/login"); }} className="mx-auto max-w-4xl">
      ...
      {report.data ? <Card><CardHeader><CardTitle>Rapor Siswa</CardTitle><CardDescription>Status {report.data.report_card.status}</CardDescription></CardHeader><CardContent className="space-y-3">{report.data.grades.map((grade) => { const summary = report.data?.report_card.summary.subjects?.find((item) => item.subject_id === grade.subject_id); return <div key={grade.grade_id} className="flex items-center justify-between rounded-lg border p-3 text-sm"><span>Subject {grade.subject_id.slice(0, 8)}</span><span>{grade.score} - {summary?.passed ? "Lulus" : "Remedial"}</span></div>; })}</CardContent></Card> : null}
    </SidebarLayout>
  );
}
