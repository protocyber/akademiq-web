...
export function ReportCardPrintDocument({ reportCardId, onReady }: { reportCardId: string; onReady?: () => void }) {
  ...
  const isLoading =
    tenant.isLoading || detail.isLoading || roster.isLoading || teachers.isLoading || homerooms.isLoading || subjects.isLoading || assignments.isLoading || studentMedia.isLoading;
  ...
  return (
    <div className="border border-black p-5 relative overflow-hidden print:border-none print:p-0">      <div className="bg-[#00c853] text-white flex items-center justify-between border border-black p-4 relative" style={{ backgroundColor: "#00c853" }}>        <div className="w-[100px] h-[100px] bg-white rounded-full border-2 border-yellow-400 p-0.5 flex-shrink-0 flex items-center justify-center shadow-md select-none print:shadow-none">
        ...
        </div>        <div className="flex-1 text-center select-none">
        ...
        <div className="bg-[#f47820] text-white py-2 px-8 rounded-full border border-black text-center font-bold tracking-widest text-sm shadow-sm select-none" style={{ backgroundColor: "#f47820" }}>
      ...
      </div>      <div className="grid grid-cols-[1.5fr_1fr] gap-8 mt-4">        <div className="flex flex-col gap-5">          <div className="text-sm font-medium select-none">
            Wali Kelas ( Ustadzah ) : <span className="font-semibold underline pl-1">{homeroomTeacherName ?? "___________________"}</span>
          </div>          {kelompokGroups.map((group) => (
            <div key={group.key} className="break-inside-avoid border border-black overflow-hidden shadow-sm">
        ...
        </div>        <div className="flex flex-col items-end gap-6">
          <div className="w-[130px] h-[173px] border-2 border-black bg-gray-50 flex items-center justify-center relative overflow-hidden shadow-inner select-none">
            {studentPhoto?.file_url ? (
              <Image src={studentPhoto.file_url} alt="Foto siswa" fill sizes="130px" className="object-cover" />
            ) : (
          ...
          </div>          <div className="w-full space-y-3 font-medium text-xs">
          ...
          </div>          <div className="w-full border border-black p-3 bg-white mt-2 select-none shadow-sm">
      ...
      </div>      <div className="mt-14 select-none">
